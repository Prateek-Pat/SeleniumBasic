...
public class ExplicitWait_Implementation {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// Explitcitly wait used on promo code box
	...
	public static void TestData(WebDriver driver, String[] arr)
	{
		...
		for(int i=0; i<items.size(); i++)
		{
			...
			if(itemNeeded.contains(trimString))
			{
				...
				if(j==arr.length) // arr.length is being used so that whenever randomly item is being
				{				  // added it's updated automatically irrespective of it's arraylength
					break;
			  }
			}
		 }
